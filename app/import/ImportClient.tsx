  ...
  if (stage === 'input') {
    return (
      <div className="mx-auto max-w-xl px-4 py-12">
        <div className="mb-6">
          <h1 className="text-xl font-semibold text-slate-900">Import from Google Sheets</h1>
          <p className="mt-1 text-sm text-slate-500">{programName}</p>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              className="border-slate-200 text-slate-900 placeholder:text-slate-400 focus:border-blue-500"
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {(['auto', 'master', 'participants', 'teams', 'mentors', 'universities', 'sponsors', 'passes'] as const).map((t) => (
                <button
                  key={t}
      ...
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <button
              ...
              className="mb-1 flex items-center gap-1.5 text-[12px] text-slate-400 hover:text-slate-600"
            >
            ...
            <p className="mt-0.5 text-sm text-slate-500">
              {activeType ? (
                <span className="text-blue-600">{SHEET_TYPE_LABELS[activeType]}</span>
              ) : (
                <span className="text-yellow-600">Type unknown — select one below</span>
              )}{' '}
          ...
          {preview.availableTabs.length > 1 && (
            <div className="shrink-0">
              ...
              <select
                ...
                disabled={isPending}
                className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[12px] text-slate-700 focus:outline-none focus:border-blue-500"
              >
        ...
        <div className="space-y-6">
          ...
          {!preview.detectedType && (
            <div className="rounded-xl border border-yellow-200 bg-yellow-50 p-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {(['master', 'participants', 'teams', 'mentors', 'universities', 'sponsors', 'passes'] as const).map((t) => (
                  <button
                    key={t}
                    onClick={() => {
                      setSheetType(t)
          ...
          {activeType && (
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
              <div className="border-b border-slate-100 px-4 py-3">
                <h2 className="text-[13px] font-medium text-slate-900">Column Mapping</h2>
                ...
                {preview.headers.map((header) => {
                  ...
                  return (
                    <div key={header} className="flex items-center gap-3 px-4 py-2.5">
                      <span className="w-48 shrink-0 truncate text-[13px] text-slate-700" title={header}>
                        {header}
                      </span>
                      ...
                      <select
                        ...
                        className="flex-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[12px] text-slate-700 focus:outline-none focus:border-blue-500"
                      >
                      ...
                      <span className="w-4 text-center text-sm">
                        {mapped ? '✅' : <span className="text-slate-300">–</span>}
                      </span>
                    </div>
                  )
                })}
          ...
          {requiredUnmapped.length > 0 && (
            <div className="flex items-start gap-2 rounded-lg border border-yellow-200 bg-yellow-50 px-3 py-2.5 text-[12px] text-yellow-700">
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <div className="border-b border-slate-100 px-4 py-3">
              <h2 className="text-[13px] font-medium text-slate-900">
                Data Preview{' '}
                <span className="text-slate-400 font-normal">(first 5 rows)</span>
              </h2>
            </div>
                ...
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50">
                    {previewHeaders.map((h) => (
                      <th
                        key={h}
                        className="px-4 py-2.5 text-left font-medium text-slate-500 whitespace-nowrap"
                      >
                        {h}
                        {columnMapping[h] && (
                    ...
                    {preview.headers.length > 6 && (
                      <th className="px-4 py-2.5 text-left text-slate-400">
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6 space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">{activeType ? SHEET_TYPE_LABELS[activeType] : ''}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3 text-center">
              <p className={`text-2xl font-bold ${hasErrors ? 'text-red-500' : 'text-slate-400'}`}>
          ...
          {hasErrors && (
            <div className="rounded-lg border border-red-200 bg-red-50 overflow-hidden">
              <button
                ...
                className="flex w-full items-center justify-between px-3 py-2.5 text-[12px] text-red-600 hover:bg-red-100"
              >
              ...
              {errorsExpanded && (
                <div className="border-t border-red-200 divide-y divide-red-100 max-h-48 overflow-y-auto">
                  {result.errors.map((e, i) => (
                    <div key={i} className="px-3 py-2 text-[12px] text-red-600">
          ...
          {/* Next steps */}
          <div className="space-y-1.5">
            <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">Next steps</p>
            <ul className="space-y-1 text-[13px] text-slate-500">
              <li>· Send invites to imported participants from <Link href="/participants" className="text-blue-600 hover:underline">/participants</Link></li>
              <li>· Review any profiles with missing data</li>
              <li>· Run mentor matching from <Link href="/matching" className="text-blue-600 hover:underline">/matching</Link></li>
            </ul>
          </div>
